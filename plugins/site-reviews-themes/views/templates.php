...
<script type="text/template" id="tmpl-glsrt-field">
    <div class="glsr-<%= custom ? 'custom' : 'review' %>-<%= tag %>">
        <% if ('textarea' === type) { %>
            <% if (tag_label) { %>
                <div class="glsr-tag-label"><%= tag_label %></div>
            <% } %>
            <p><%= text %><% if (text !== value) { %>&hellip; <a href=""><?= _x('Read more', 'admin-text', 'site-reviews-themes'); ?></a><% } %></p>
        <% } else { %>
            <%= value %>
        <% } %>
    </div>
...
<script type="text/template" id="tmpl-glsrt-input-colorpicker">
    <input type="text" class="glsr-color-picker glsr-input-value"
        data-alpha-enabled="true"
        data-tippy-content="<%= _.escape(tooltip) %>"
        data-tippy-allowHTML="true"
        value="<%= value %>"
    >
</script>

<script type="text/template" id="tmpl-glsrt-input-boxshadow">
    <div class="glsrt-boxshadow glsrt-multifield">
        <% _.each(value.split('|'), (val, i) => { %>
            <% if (i < 4) { %>
                <input type="number"
                    data-linked
                    data-tippy-content="<%= _.escape(tooltips[i]) %>"
                    placeholder="0"
                    value="<%= val %>"
                >
            <% } else { %>
...
<script type="text/template" id="tmpl-glsrt-input-dimensions">
    <div class="glsrt-dimensions glsrt-multifield">
        <% _.each(value.split('|'), (val, i) => { %>
            <% if (i < 4) { %>
                ...
                >
            <% } else if (i === 4) { %>
                <select data-linked data-tippy-content="<%= _.escape(tooltips[i]) %>">
                    <option value="%" <% if (val === '%') { %>selected<% } %>>%</option>
                    <option value="px" <% if (val === 'px') { %>selected<% } %>>px</option>
                    <option value="em" <% if (val === 'em') { %>selected<% } %>>em</option>
                    <option value="rem" <% if (val === 'rem') { %>selected<% } %>>rem</option>
                </select>
            <% } else { %>
                <button type="button"
                    class="components-button dashicons-admin-links dashicons-before has-icon is-small <% if (+val) { %>is-pressed<% } %>"
...
<script type="text/template" id="tmpl-glsrt-input-excerpt">
    <div class="glsrt-excerpt glsrt-multifield">
        <% _.each(value.split('|'), (val, i) => { %>
            <% if (i === 0) { %>
                ...
                >
            <% } else { %>
                <select data-linked data-tippy-content="<%= _.escape(tooltip) %>">
                    <option value="chars" <% if (val === 'chars') { %>selected<% } %>><?= _x('characters', 'admin-text', 'site-reviews-themes'); ?></option>
                    <option value="words" <% if (val === 'words') { %>selected<% } %>><?= _x('words', 'admin-text', 'site-reviews-themes'); ?></option>
                </select>
            <% } %>
        <% }) %>
    </div>
...
<script type="text/template" id="tmpl-glsrt-input-number">
...
<script type="text/template" id="tmpl-glsrt-input-range">
    <div class="glsrt-input">
...
<script type="text/template" id="tmpl-glsrt-input-ratingcolors">
...
<script type="text/template" id="tmpl-glsrt-input-text">
...
<script type="text/template" id="tmpl-glsrt-input-typography">
    <div class="glsrt-typography glsrt-multifield">
        <% _.each(value.split('|'), (val, i) => { %>
            ...
            <% } else if (i === 2) { %>
                <select data-linked data-tippy-content="<%= _.escape(tooltips[i]) %>">
                    <option value="ch" <% if (val === 'ch') { %>selected<% } %>>ch</option>
                    <option value="em" <% if (val === 'em') { %>selected<% } %>>em</option>
                    <option value="ex" <% if (val === 'ex') { %>selected<% } %>>ex</option>
                    <option value="pt" <% if (val === 'pt') { %>selected<% } %>>pt</option>
                    <option value="px" <% if (val === 'px') { %>selected<% } %>>px</option>
                    <option value="rem" <% if (val === 'rem') { %>selected<% } %>>rem</option>
                </select>
            <% } else { %>
...
<script type="text/template" id="tmpl-glsrt-nav">
    <div class="components-panel__header edit-post-sidebar__panel-tabs" tabindex="-1">
        <ul></ul>
    </div>
</script>

<script type="text/template" id="tmpl-glsrt-nav-item">
    <button type="button" aria-label="<%= label %>" class="components-button edit-post-sidebar__panel-tab" data-group="<%= group %>">
        <%= label %>
    </button>
</script>

<script type="text/template" id="tmpl-glsrt-preview">
    <% if (!has_reviews) { %>
    ...
    <div class="glsrt-preview glsr" data-theme>
        <% if ('carousel' === _.get(layout, 'display_as')) { %>
            <% if ('splide' === swiper) { %>
            ...
            <% } %>
        <% } else if ('grid' === _.get(layout, 'display_as')) { %>
            <div class="gl-reviews gl-display-grid gl-grid-<%= _.get(layout, 'max_columns') %>"></div>
        <% } else { %>
...
<script type="text/template" id="tmpl-glsrt-preview-placeholder">
...
<script type="text/template" id="tmpl-glsrt-select">
    <select data-tippy-content='<%= _.escape(tooltip) %>' data-tippy-allowHTML="true">
        <% _.each(options, function (optLabel, optValue) { %>
            <option value="<%= optValue %>" <% if (value == optValue) { %>selected<% } %>><%= optLabel %></option>
        <% }) %>
    </select>
</script>

<script type="text/template" id="tmpl-glsrt-setting">
    <% if (label) { %><label><%= label %></label><% } %>
    <div class="glsr-input wp-clearfix"></div>
</script>

<script type="text/template" id="tmpl-glsrt-setting-panel">
    <div class="components-panel__body" data-controls="<%= section %>">
        <div class="components-panel__body-title">
            <button aria-expanded="false" class="components-button components-panel__body-toggle dashicons-before <%= dashicon %>" type="button">
                ...
                <span class="components-panel__label"><%= label %></span>
            </button>
        </div>
...
<script type="text/template" id="tmpl-glsrt-toolbar">
    ...
    <% if (!_.isEmpty(model) && is_container) { %>
        <div class="glsrt-toolbar-group">
            <button  type="button" class="components-button has-icon"
                aria-pressed="<%= model.direction === 'row' %>"
        ...
        <div class="glsrt-toolbar-group">
            <button type="button" class="components-button has-icon <% if (model.wrap) { %>is-pressed<% } %>" 
                aria-pressed="<%= model.wrap %>"
                data-action="flex-wrap"
                data-tippy-content="<% if ('row' === model.direction) { %><?= _x('Allow the fields to flow onto multiple rows', 'admin-text', 'site-reviews-themes'); ?><% } else { %><?= _x('Allow the fields to flow onto multiple columns', 'admin-text', 'site-reviews-themes'); ?><% } %>">
                <svg aria-hidden="true" focusable="false" width="24" height="24" viewBox="0 0 24 24">
                    <% if ('row' === model.direction) { %>
        ...
        <div class="glsrt-toolbar-group">
            <button  type="button" class="components-button has-icon <% if (model.align === 'start') { %>is-pressed<% } %>"
                aria-pressed="<%= model.align === 'start' %>"
                data-action="align"
                data-value="start"
                <% if ('row' === model.direction) { %>
            ...
            <button type="button" class="components-button has-icon <% if (model.align === 'center') { %>is-pressed<% } %>"
                aria-pressed="<%= model.align === 'center' %>"
                data-action="align"
                data-value="center"
            ...
            <button type="button" class="components-button has-icon <% if (model.align === 'end') { %>is-pressed<% } %>" 
                aria-pressed="<%= model.align === 'end' %>"
                data-action="align"
                data-value="end"
                <% if ('row' === model.direction) { %>
    ...
    <% } %>
    <% if (!_.isEmpty(model) && !is_root) { %>
        <div class="glsrt-toolbar-group">
            <button type="button" class="components-button has-icon <% if (model.flex === 'shrink') { %>is-pressed<% } %>"
                aria-pressed="<%= model.flex === 'shrink' %>"
                data-action="flex"
                data-value="shrink"
            ...
            <button type="button" class="components-button has-icon <% if (model.flex === 'grow') { %>is-pressed<% } %>" 
                aria-pressed="<%= model.flex === 'grow' %>" 
                data-action="flex" 
                data-value="grow"
            ...
            <button type="button" class="components-button has-icon <% if (model.flex === 'none') { %>is-pressed<% } %>" 
                aria-pressed="<%= model.flex === 'none' %>" 
                data-action="flex" 
                data-value="none"
    ...
    <% if (!_.isEmpty(model) && !is_container) { %>
        <div class="glsrt-toolbar-group">
            <button type="button" class="components-button has-icon <% if ('large' === model.text) { %>is-pressed<% } %>" 
                data-action="text" 
                data-value="large" 
                aria-pressed="<%= 'large' === model.text %>" 
            ...
            <button type="button" class="components-button has-icon <% if ('normal' === model.text) { %>is-pressed<% } %>" 
                data-action="text" 
                data-value="normal" 
                aria-pressed="<%= 'normal' === model.text %>" 
            ...
            <button type="button" class="components-button has-icon <% if ('small' === model.text) { %>is-pressed<% } %>" 
                data-action="text" 
                data-value="small" 
                aria-pressed="<%= 'small' === model.text %>" 
        ...
        <div class="glsrt-toolbar-group">
            <button type="button" class="components-button has-icon <% if (model.is_bold) { %>is-pressed<% } %>" 
                aria-pressed="<%= model.is_bold %>" 
                data-action="is_bold" 
        ...
        <div class="glsrt-toolbar-group">
            <button type="button" class="components-button has-icon <% if (model.is_italic) { %>is-pressed<% } %>" 
                aria-pressed="<%= model.is_italic %>" 
        ...
        <div class="glsrt-toolbar-group">
            <button type="button" class="components-button has-icon <% if (model.is_uppercase) { %>is-pressed<% } %>" 
                aria-pressed="<%= model.is_uppercase %>" 
        ...
        <div class="glsrt-toolbar-group">
            <button type="button" class="components-button has-icon <% if (model.is_hidden) { %>is-pressed<% } %>" 
                aria-pressed="<%= model.is_hidden %>" 
    ...
    <% } %>
    <% if (!_.isEmpty(model) && is_container && parent.direction === 'row') { %>
        <div class="glsrt-toolbar-group">
