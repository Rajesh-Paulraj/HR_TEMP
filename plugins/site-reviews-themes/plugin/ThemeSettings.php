...
class ThemeSettings extends ThemeBuilder
{
    ...
    {
        $settings = Arr::convertFromDotNotation($this->sections());
        if (empty($this->toArray())) { // get and store the saved settings
            return $settings;
        }
        $config = Arr::convertFromDotNotation(glsr(Application::class)->config('theme-settings'));
    ...
    {
        $data = [];
        $groups = Arr::convertFromDotNotation($this->sections());
