...
class Gatekeeper
{
    ...
    public function createDependenciesNotice()
    {
        if ($errors = $this->getDependencyErrors()) {
            $message = _nx_noop('%s requires the latest version of', '%s requires the latest version of the following plugins:', 'admin-text', 'site-reviews-authors');
            $message = sprintf(translate_nooped_plural($message, count($errors), 'site-reviews-authors'), '<strong>'.$this->addonName.'</strong>');
    ...
    public function createUnsupportedNotice()
    {
        if ($errors = $this->getUnsupportedErrors()) {
            $message = _nx_noop('%s needs an update to work with', '%s needs an update to work with the following plugins:', 'admin-text', 'site-reviews-authors');
            $message = sprintf(translate_nooped_plural($message, count($errors), 'site-reviews-authors'), '<strong>'.$this->addonName.'</strong>');
